...
import { Play, Flame, Trophy, Clock, Target, ChartColumn, ListChecks, Layers } from 'lucide-react'
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap items-end justify-between gap-4">
        <div>
          <p className="text-[13px] font-medium text-ink-3">{formatDisplayDate(today)}</p>
          <h1 className="mt-1 text-[26px] font-bold tracking-[-0.03em] text-ink sm:text-3xl">
            Good {getGreeting()}, Vijay
          </h1>
          <p className="mt-1 text-sm text-ink-2">Here's your progress today.</p>
        </div>
      ...
      <div className="stagger grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-12 xl:gap-5">
          ...
          <div className="mt-4 flex flex-1 flex-col items-start justify-between gap-6 sm:flex-row sm:items-center">
            <div className="flex min-w-0 flex-1 flex-col self-stretch">
              <div className="flex items-end gap-1.5">
                <span className="text-[64px] font-extrabold leading-none tracking-[-0.04em] text-ink tabular">
                  {stats.percentage}
                </span>
                <span className="pb-1.5 text-[26px] font-bold leading-none text-ink-3">%</span>
              </div>
              <p className="mt-3 text-sm text-ink-2">
                ...
                {stats.remaining > 0 && <span className="text-ink-3"> · {stats.remaining} to go</span>}
              </p>

              <div className="mt-4 flex flex-wrap gap-2">
                <span className="inline-flex items-center gap-1.5 rounded-full border border-sky/25 bg-sky/10 px-3 py-1 text-[12px] font-medium text-sky tabular">
                  <Clock size={12} />
                  {formatMinutes(record?.totalTimeMinutes ?? 0)}
                </span>
                <span className="inline-flex items-center gap-1.5 rounded-full border border-mint/20 bg-mint/10 px-3 py-1 text-[12px] font-medium text-mint tabular">
                  <Target size={12} />
            ...
            <div className="shrink-0 self-center">
              <CircularProgress percentage={stats.percentage} size={172} strokeWidth={13}>
                <span className="text-xl font-bold leading-none text-ink tabular">
                  {record?.totalProblems ?? 0}
                </span>
                <span className="mt-1 text-[11px] text-ink-3">problems solved</span>
              </CircularProgress>
            </div>
          </div>
        </Card>
        ...
        <Card className="flex flex-col xl:col-span-5">
          ...
          <div className="mt-3 flex items-end gap-2">
            <span className="text-[46px] font-extrabold leading-none tracking-[-0.03em] text-ink tabular">
              {state.streak.current}
            </span>
            <span className="pb-1.5 text-[13px] font-medium text-ink-3">
              {state.streak.current === 1 ? 'day' : 'days'}
            </span>
          </div>
          <div className="mt-auto pt-5">
            <p className="mb-2 text-[11px] font-semibold uppercase tracking-[0.09em] text-ink-3">
              Last 7 days
            </p>
                ...
                return (
                  <span
                    ...
                    className={cn(
                      ...
                      productive ? 'bg-accent' : 'border border-line bg-surface-2'
                    )}
                  />
                )
              })}
            ...
            <div key={topic.name} className="flex items-center gap-3">
              ...
              <div className="min-w-0 flex-1">
                <div className="flex items-center justify-between gap-2">
                  ...
                  <span className="shrink-0 text-[12px] font-semibold text-ink-3 tabular">{topic.percentage}%</span>
                </div>
